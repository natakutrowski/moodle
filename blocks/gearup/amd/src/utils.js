 ...
 */
export async function getPreference(name, defaultValue = null) {
    let value = null;
    if (typeof UserRepository.getUserPreference === 'function') {
        value = await UserRepository.getUserPreference(name);
    } else {
        const prefs = (await ws('core_user_get_user_preferences', {name})).preferences || [];
...
export async function setPreference(name, value) {
    if (typeof UserRepository.setUserPreference === 'function') {
        return UserRepository.setUserPreference(name, value);
    }
    return ws('core_user_set_user_preferences', {
        preferences: [{
            name: name,
            value: value,
        }]
