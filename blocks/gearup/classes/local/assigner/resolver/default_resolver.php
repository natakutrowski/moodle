 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class default_resolver implements resolver, resolver_for_action {
    ...
    protected function get_type_name_from_class($classname) {
        return str_replace('block_gearup\\local\\assigner\\type\\', '', $classname);
    }
    ...
    public function get_types(): array {
        $this->init_types();
        $typenames = $this->lm->get_assigner_types();
    ...
    /**
     * Get the types compatible with action.
     *
     * @param action $action The action.
     * @return type[]
     */
    public function get_types_compatible_with_action(action $action): array {
        $this->init_types();
        return array_filter($this->types, function ($type) use ($action) {
            return $type instanceof type_with_action_consumption
                && $type->is_action_compatible($action);
        });
    }
    ...
    public function get_types_compatible_with_event(\core\event\base $event): array {
        $this->init_types();
        return array_filter($this->types, function ($type) use ($event) {
            return $type instanceof type_with_event_consumption
                && $type->is_event_compatible($event);
        });
    }
    ...
    protected function init_types() {
        if (!isset($this->types)) {
            $types = [];
            $candidates = array_keys(\core_component::get_component_classes_in_namespace('block_gearup', 'local\assigner\type'));
