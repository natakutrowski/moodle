...
use block_gearup\local\assigner\resolver\resolver_for_action;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class default_processor implements action_processor, all_processor, backup_processor, event_processor, restore_processor {
    ...
    public function process_action(action $action) {
        if (!$this->assignertyperesolver instanceof resolver_for_action) {
            return;
        }

        $types = $this->assignertyperesolver->get_types_compatible_with_action($action);
        if (empty($types)) {
            return;
        }

        $iterator = $this->repository->get_assigners_of_types_from_unassigned_missions(
            $types,
            $action->get_user_id(),
            $action->get_context()
        );
        foreach ($iterator as [$assigner, $mission]) {
            $type = $assigner->get_type();
            if ($type instanceof type_with_action_consumption) {
                if (!$type->is_action_passing_constraints($action, $assigner)) {
                    continue;
                }
                $this->missionoperator->sync_assigner_from_action($mission, $assigner, $action);
            }
        }
    }
