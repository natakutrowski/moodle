...
class mission_exporter extends \core\external\exporter {
    ...
    /** Quest edges. */
    const WITH_QUEST_EDGES = 'questedges';

    /** @var mission The mission. */
    protected $mission;
    /** @var \block_gearup\local\url\url_resolver */
    ...
    protected static function define_other_properties() {
        $edgequest = [
            ...
            'manageurl' => ['type' => PARAM_URL],
            'type' => ['type' => ['isquest' => ['type' => PARAM_BOOL]]],
            'visual' => [
                'optional' => true,
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $manageurl = null;
        if ($canmanage) {
            $manageurl = $this->get_url_resolver()->reverse('mission', ['missionid' => $mission->get_id()])->out(false);
        }
        ...
        $edges = null;
        if ($canmanage && $missionhelper->is_a_quest($mission) && $this->is_with(static::WITH_QUEST_EDGES)) {
            $edges = $this->get_edges_properties($output);
        }
     ...
     *
     * @param renderer_base $output The renderer.
     * @return array|null The edges properties, or null if there are no edges.
     */
    protected function get_edges_properties($output) {
        $mission = $this->mission;
        $repository = di::get('repository');

        $missionhelper = $this->related['mission_helper'];
        if (!$missionhelper->is_a_quest($this->mission)) {
            return null;
        }

        $edges = $repository->get_quest_edges_to_quests($mission);
        $comesfromquests = array_map(function ($edge) {
            return $edge->from;
        }, array_filter($edges, function ($edge) use ($mission) {
            return $edge->to->get_id() === $mission->get_id();
        }));
        $leadstoquests = array_map(function ($edge) {
            return $edge->to;
        }, array_filter($edges, function ($edge) use ($mission) {
            return $edge->from->get_id() === $mission->get_id();
        }));

        $data = [
            'comesfromquests' => array_values(array_map(function ($mission) use ($output) {
                return $this->get_mission_edge_properties($mission, $output);
            }, $comesfromquests)),
            'leadstoquests' => array_values(array_map(function ($mission) use ($output) {
                return $this->get_mission_edge_properties($mission, $output);
            }, $leadstoquests)),
            'hasanyquests' => !empty($comesfromquests) || !empty($leadstoquests),
            'hasleadstoquests' => !empty($leadstoquests),
            'hascomesfromquests' => !empty($comesfromquests),
        ];
    ...
    /**
     * Get the properties for a mission edge.
     *
     * @param mission $mission The mission.
     * @param renderer_base $output The renderer.
     * @return array The properties of the edge.
     */
    protected function get_mission_edge_properties($mission, $output) {
        $missionhelper = $this->related['mission_helper'];
        $urlresolver = $this->get_url_resolver();

        // This is an edge case, it really should not happen because we're not expecting to link quests across contexts,
        // but in the event we do, we must use the appropriate URL resolver, else the link will not work.
        if ($this->mission->get_context()->id != $mission->get_context()->id) {
    ...
    protected function get_url_resolver() {
        if (isset($this->related['url_resolver'])) {
            return $this->related['url_resolver'];
        }
        if (!isset($this->urlresolver)) {
            $this->urlresolver = di::get('url_resolver_factory')->get_resolver_for_context($this->mission->get_context(), null);
        }
    ...
    public function with_quest_edges(): self {
        $this->with(static::WITH_QUEST_EDGES);
        return $this;
    }
