...
class tracker_exporter extends \core\external\exporter {
    ...
    public function __construct(int $userid, $related = []) {
        ...
        if (!isset($related['url_resolver'])) {
            $related['url_resolver'] = di::get('url_resolver_factory')
                ->get_resolver_for_context($related['context'], $related['context']);
        }
    ...
    protected static function define_other_properties() {
        return [
            ...
            'paramshowsections' => ['type' => PARAM_RAW],
            'hasparamshowsections' => ['type' => PARAM_BOOL],
        ];
    }
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $sections = [];
        if ($hasany) {
            $sectionstoshow = $this->normalise_section_names($this->related['showsections'] ?? []);
            $sectionsorder = $sectionstoshow ?: tracker::get_missions_order();
        ...
        return [
            ...
            'paramshowsections' => implode(',', array_map(function ($section) {
                return str_replace('block_gearup\\local\\mission\\', '', $section);
            }, $sectionstoshow)),
            'hasparamshowsections' => !empty($sectionstoshow),
        ];
    }

    /**
     * Normalise a list of section names.
     *
     * @param array $sections The sections.
     * @return string[] The normalised sections.
     */
    protected function normalise_section_names(array $sections) {
        $normalised = [];
        foreach ($sections as $section) {
            if (strpos($section, 'achievement') === 0) {
                $normalised[] = achievement::class;
            } else if (strpos($section, 'quest') === 0) {
                $normalised[] = quest::class;
            } else if (strpos($section, 'challenge') === 0) {
                $normalised[] = challenge::class;
            } else if (strpos($section, 'streak') === 0) {
                $normalised[] = streak::class;
            }
        }
