...
class mission_instance_exporter extends \core\external\exporter {
    ...
    protected function get_storyline_values($user) {
        ...
        if ($mh->is_a_quest($mission)) {
            ...
            // Description.
            $isunread = $needsattention && !$hasstarted;
            foreach ($this->convert_storyline_to_messages($description) as $message) {
                $messages[] = $this->convert_storyline_message_to_contenttype($message, $isunread);
            }

            // Instructions.
            $isunread = $needsattention && !$hascompleted;
            foreach ($this->convert_storyline_to_messages($instructions) as $message) {
                $messages[] = $this->convert_storyline_message_to_contenttype($message, $isunread);
            }
            $dialoguecontainsobjectives = count(array_filter($messages, function($message) {
                return $message['contenttype'] === 'objectives';
            })) > 0;
            if ($instructions && !$dialoguecontainsobjectives) {
                $messages[] = ['contenttype' => 'objectives', 'isunread' => $isunread];
            }

            // Feedback.
            $isunread = $needsattention && !$isended;
            foreach ($this->convert_storyline_to_messages($feedback) as $message) {
                $messages[] = $this->convert_storyline_message_to_contenttype($message, $isunread);
            }
            $dialoguecontainsrewards = count(array_filter($messages, function($message) {
                return $message['contenttype'] === 'rewards';
            })) > 0;
            if ($feedback && !$dialoguecontainsrewards) {
                $messages[] = ['contenttype' => 'rewards', 'isunread' => $isunread];
            }
            ...
            foreach ($messages as $message) {
                $isunreadfromhere = $isunreadfromhere || ($message['isunread'] ?? false);
                $message = [
                    'contenttype' => $message['contenttype'],
                    'content' => $message['content'] ?? null,
                    'ismessage' => $message['contenttype'] === 'message',
                    'isobjectives' => $message['contenttype'] === 'objectives',
                    'isrewards' => $message['contenttype'] === 'rewards',
                ];
                if ($isunreadfromhere) {
