...
class mission_operator {
    ...
    public function __construct($repository, $missionhelper, $completionratiocalculator,
    ...
    public function delete_mission(mission $mission) {
        ...
        if (!empty($objids)) {
            list($insql, $inparams) = $DB->get_in_or_equal($objids);
    ...
    public function sync_assigner(mission $mission, assigner $assigner) {
        ...
        list($userssql, $usersparams) = $type->get_elligible_users_sql($assigner, $mission);
    ...
    public function sync_assigner_from_event(mission $mission, assigner $assigner, \core\event\base $event) {
        ...
        list($userssql, $usersparams) = $type->get_elligile_users_sql_from_event($event, $assigner, $mission);
        $this->sync_assigner_with_users_sql($mission, $assigner, $userssql, $usersparams);
    }

    protected function sync_assigner_with_users_sql(mission $mission, assigner $assigner,
            $userssql, $usersparams) {
        global $CFG, $DB;
