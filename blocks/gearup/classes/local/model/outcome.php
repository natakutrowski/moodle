 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class outcome extends persistent {

    const TABLE = 'block_gearup_outcome';
     ...
     *
     * @return \block_gearup\local\outcome\type\type
     */
    protected function get_type_object() {
        $type = di::get('outcome_type_resolver')->get_type($this->raw_get('type'));
        if (!$type) {
    ...
    protected function validate_type($value) {
        try {
            $type = di::get('outcome_type_resolver')->get_type($value);
