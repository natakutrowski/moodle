 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quest_narrators_repository implements repository_with_context {
    ...
    public function __construct(cache $cache, api_client $apiclient) {
        ...
        $this->maxlocal = di::get('lm')->max_quest_narrators();
    }
    ...
    protected function load() {
        if (!isset($this->visuals)) {
            $this->visuals = array_reduce($this->cache->get('quest_narrators'), function ($carry, $visual) {
                $narrator = new static_narrator(
                    $visual->id,
                    $visual->url
                );
                if (!empty($visual->speaking_frames)) {
                    $frames = array_map(function ($frame) {
                        return new moodle_url($frame);
                    }, $visual->speaking_frames);
                    $narrator->set_speaking_frames($frames);
                }
                $carry[$visual->id] = $narrator;
                return $carry;
            }, []);
        }
    }
