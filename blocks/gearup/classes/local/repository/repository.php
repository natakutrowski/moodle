...
class repository {
    ...
    public function __construct(
        objective_resolver $objtyperesolver,
        outcome_resolver $outcometyperesolver,
     ...
     * @return achievement[]
     */
    public function get_achievement_instances_by_subject_id(
        int $subjectid,
        $state = null,
    ...
    /**
     * Get assigners of types from unassigned missions.
     *
     * @param \block_gearup\local\assigner\type\type[] $types
     * @param int $userid
     * @param context $context
     * @return array [$assigner, $mission] Tuple of assigner and mission.
     */
    public function get_assigners_of_types_from_unassigned_missions(array $types, int $userid, context $context): iterable {
        global $DB;
        ...
        $params = $typeinparams + [
            'mstateactive' => mission::STATE_ACTIVE,
            'userid' => $userid,

            // Filter the context as per the mission contexts. At this point, if mission do not share a context path.
            // We select where the context is a descendant of the mission's, or the mission's is a descendant of the context,
        ...
        foreach ($recordset as $record) {
            ...
            yield [
                new persisted_assigner($assignermodel, $this->assignertyperesolver),
                $this->make_mission_from_model($missionmodel),
            ];
        }
        $recordset->close();
    }
     ...
     * @param int $limit
     */
    public function get_challenge_instances_by_subject_id(
        int $subjectid,
        $state = null,
        ?context $context = null,
        ?array $order = null,
    ...
    public function get_incomplete_objective_instances_of_types(array $types, int $userid, context $context) {
        ...
        [$insql, $inparams] = $DB->get_in_or_equal($typenames, SQL_PARAMS_NAMED);
    ...
    public function get_instance_by_subject_id(int $missionid, int $subjectid) {
        $missioninsts = $this->fetch_instances_select("mi.missionid = ? AND mi.subjectid = ?",
            [$missionid, $subjectid],
    ...
    public function get_instances(
        ...
        if ($state !== null) {
            [$statesql, $stateparams] = $DB->get_in_or_equal((array) $state, SQL_PARAMS_NAMED);
    ...
    public function get_instances_by_ids($missioninstids) {
        ...
        [$insql, $inparams] = $DB->get_in_or_equal($missioninstids, SQL_PARAMS_NAMED);
    ...
    public function count_visible_instances_in(int $subjectid, context $context, ?string $type, ?array $state) {
        ...
        $statewhere = '1=1';
        if ($state !== null) {
            [$insql, $inparams] = $DB->get_in_or_equal((array) $state);
    ...
    /**
     * Get the quest edges to quests.
     *
     * @param quest $mission.
     * @return object[] Containing the keys 'from' and 'to' containing the quest instance.
     */
    public function get_quest_edges_to_quests(quest $mission) {
        global $DB;

        $context = $mission->get_context();
        $outcometyperesolver = $this->outcometyperesolver;
        $assignertyperesolver = $this->assignertyperesolver;

        $edges = [];
        $missionsmap = [$mission->get_id() => $mission];

        $afields = assigner_model::get_sql_fields('a', 'a_');
        $ofields = outcome_model::get_sql_fields('o', 'o_');
        $mfields = mission_model::get_sql_fields('m', 'm_');

        $typenames = $this->filter_compatible_types_by_name($outcometyperesolver, ['assign_quest']);
        if (!empty($typenames)) {
            $inparams = [];
            [$intypessql, $inparams] = $DB->get_in_or_equal($typenames, SQL_PARAMS_NAMED);
            $sql = "SELECT $ofields, $mfields
                      FROM {block_gearup_outcome} o
                      JOIN {block_gearup_mission} m
                        ON o.missionid = m.id
                     WHERE o.type $intypessql
                       AND m.contextid = :contextid
                       AND m.type = :type
                       AND m.state = :state";
            $params = [
                ...
                'state' => $mission->get_state(), // Only get active if active, or archived if archived.
            ] + $inparams;
            $records = $DB->get_records_sql($sql, $params);
            foreach ($records as $record) {
                ...
                $outcome = new persisted_outcome($outcomemodel, $outcometyperesolver);
                $outcomemission = $this->make_mission_from_model($missionmodel);
                $missionsmap += [$outcomemission->get_id() => $outcomemission];

                $config = $outcome->get_type_config();
                $targetmissionid = (int) ($config->questid ?? $config->missionid ?? null);
                $sourcemissionid = $outcomemission->get_id();
                if (!$targetmissionid || ($targetmissionid !== $mission->get_id() && $sourcemissionid !== $mission->get_id())) {
                    continue;
                }

                $edges[] = ['from' => $sourcemissionid, 'to' => $targetmissionid];
            }
        }

        $typenames = $this->filter_compatible_types_by_name($assignertyperesolver, ['quest_state']);
        if (!empty($typenames)) {
            $inparams = [];
            [$intypessql, $inparams] = $DB->get_in_or_equal($typenames, SQL_PARAMS_NAMED);
            $sql = "SELECT $afields, $mfields
            ...
            foreach ($records as $record) {
                ...
                $assigner = new persisted_assigner($assignermodel, $assignertyperesolver);
                $assignermission = $this->make_mission_from_model($missionmodel);
                $missionsmap += [$assignermission->get_id() => $assignermission];

                $config = $assigner->get_type_config();
                $sourcemissionid = (int) ($config->missionid ?? null);
                $targetmissionid = $assignermission->get_id();
                if (!$targetmissionid || ($targetmissionid !== $mission->get_id() && $sourcemissionid !== $mission->get_id())) {
                    continue;
                }
        ...
        $edgeids = array_unique($edgeids);
        $unknownids = array_diff($edgeids, array_keys($missionsmap));
        if (!empty($unknownids)) {
            $inparams = [];
            [$insql, $inparams] = $DB->get_in_or_equal($unknownids, SQL_PARAMS_NAMED);
            foreach (mission_model::get_records_select("id $insql", $inparams) as $missionmodel) {
                $missionsmap[$missionmodel->get('id')] = $this->make_mission_from_model($missionmodel);
            };
        }

        $finaledges = [];
        foreach ($edges as $edge) {
            $frommission = $missionsmap[$edge['from']] ?? null;
            $tomission = $missionsmap[$edge['to']] ?? null;
     ...
     * @param int $limit
     */
    public function get_quest_instances_by_subject_id(
        int $subjectid,
        $state = null,
        ?context $context = null,
        ?array $order = null,
     ...
     * @return mission_instance[]
     */
    public function get_type_instances_by_subject_id(
        string $type,
        int $subjectid,
        $state = null,
        ?context $context = null,
        ...
        return $this->fetch_instances_by_subject_id($subjectid,
            mission_model::get_internal_type($type),
            $state,
            $context,
            $order,
            $offset,
     ...
     * @return string[] The names of the compatible types.
     */
    protected function filter_compatible_types_by_name($resolver, $typenames) {
        if (!method_exists($resolver, 'get_types') || !method_exists($resolver, 'get_type_name')) {
            return [];
        }

        $alltypes = $resolver->get_types();
        $alltypenames = array_map(function ($type) use ($resolver) {
            return $resolver->get_type_name($type);
        }, $alltypes);
     ...
     * @return mission_instance[]
     */
    protected function fetch_instances_by_subject_id(
        int $subjectid,
        ?int $type = null,
        $state = null,
        ?context $context = null,
    ...
    protected function make_instances_from_models(array $missioninsts, array $missions) {
        ...
        [$inidsql, $inidparams] = $DB->get_in_or_equal(array_keys($missions), SQL_PARAMS_NAMED);
        [$inmistidsql, $inmistidparams] = $DB->get_in_or_equal(array_keys($missioninsts), SQL_PARAMS_NAMED);
