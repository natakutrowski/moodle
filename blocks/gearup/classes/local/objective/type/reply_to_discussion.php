 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class reply_to_discussion implements type {
    ...
    public function get_config_form_extender(mission $mission, ?objective $objective): \block_gearup\local\form\extender {
        return new reply_to_discussion_config_form_extender();
    }

    public function get_display_name(): \lang_string {
        return new \lang_string('typereplytodiscussion', 'block_gearup');
    }

    public function get_short_description(): \lang_string {
        return new \lang_string('typereplytodiscussiondesc', 'block_gearup');
    }
    ...
    public function is_action_passing_constraints(action $action, objective_instance $instance,
        ...
        if ($uniqueness > 0) {
            $contextid = (int) $action->get_context()->id;
            $discid = $uniqueness == 1 ? $action->get_discussion_id() : null;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class reply_to_discussion_config_form_extender implements extender {
