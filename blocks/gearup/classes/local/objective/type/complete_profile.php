...
class complete_profile implements type, type_with_state_initialisation, type_with_supporting_url {
    ...
    public function get_config_form_extender(mission $mission, ?objective $objective): \block_gearup\local\form\extender {
        return new complete_profile_config_form_extender();
    }
    ...
    protected function has_completed_profile(objective_instance $instance, int $userid): bool {
        ...
        $fields = $config->f ?? [];
        $user = $this->get_user_with_fields($userid, $fields);

        // If we didn't find the user, this is likely due to the deletion of a profile field.
        // TODO Observe when a profile field has been deleted, update the objective config and reevaluate the instances.
        if ($user === false) {

            // Check if there are profile fields.
            ...
            list($insql, $inparams) = $DB->get_in_or_equal($profilefields);
