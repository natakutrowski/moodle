 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class complete_section implements type, type_with_state_initialisation, type_with_supporting_url, type_with_update_after_restore,
        has_availability_info, has_availability_info_for_context {
    ...
    public function initialise_state(objective_instance $instance, mission_instance $missioninst) {
        ...
        $seen = [];
        if ($sectionnum !== null) {
            if ($this->is_section_completed($modinfo, $completioninfo, $sectionnum)) {
                $counter = 1;
            }
        } else {
    ...
    public function is_action_passing_constraints(action $action, objective_instance $instance,
        ...
        if (!$this->is_section_completed($modinfo, $completioninfo, $actionsectionnum)) {
            return false;
        }
     ...
     * @return bool
     */
    protected function is_section_completed($modinfo, $completioninfo, $sectionnum) {
        global $CFG;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class config_form implements extender, extender_with_supporting_url_modes {
    ...
    public function definition($mform): array {
        ...
        $els[] = $mform->addElement('select', 'cd_whichsection', get_string('elligiblesection', 'block_gearup'),
            $whichsectionoptions);
        ...
        if ($incourse) {
            ...
            $els[] = $mform->addElement('select', 'cd_sectionnum', get_string('choosesection', 'block_gearup'),
