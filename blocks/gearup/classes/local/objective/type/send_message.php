 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class send_message implements has_availability_info, type {
    ...
    public function consume_action(action $action, objective_instance $instance, mission_instance $missioninst) {
        ...
        if (!empty($config->uniqueness)) {
            $state->recipients[] = $action->get_target_user_id();
    ...
    public function get_config_form_extender(mission $mission, ?objective $objective): \block_gearup\local\form\extender {
        return new send_message_config_form_extender();
    }

    public function get_display_name(): \lang_string {
        return new \lang_string('typesendmessage', 'block_gearup');
    }

    public function get_short_description(): \lang_string {
        return new \lang_string('typesendmessagedesc', 'block_gearup');
    }

    public function is_action_compatible(action $action): bool {
        return $action instanceof message_sent;
    }
    ...
    public function is_action_passing_constraints(
        ...
        $sendingto = $action->get_target_user_id();
        $issendingtoother = $action->get_user_id() !== $sendingto;
        if (!$issendingtoother) {
            return false;
        }
        ...
        // When each recipient must be unique.
        if (!empty($config->uniqueness)) {
            if (in_array($sendingto, $state->recipients)) {
                return false;
            }
        }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class send_message_config_form_extender implements extender {

    public function definition($mform): array {
        $els = [];

        $els[] = $mform->addElement('advcheckbox', 'cd_uniqueness', get_string('todifferentpeople', 'block_gearup'));
        $mform->addHelpButton('cd_uniqueness', 'todifferentpeople', 'block_gearup');
