 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class participate_in_forum implements type {
    ...
    public function consume_action(action $action, objective_instance $instance, mission_instance $missioninst) {
        ...
        if (!empty($config->uniqueness)) {
            $discid = $this->get_discussion_id_from_action($action);
            $state = $this->get_normalized_state($instance);
            $state->seen[] = [
                'contextid' => (int) $action->get_context()->id,
                'discid' => $discid,
            ];
    ...
    public function get_config_form_extender(mission $mission, ?objective $objective): \block_gearup\local\form\extender {
        return new participate_in_forum_config_form_extender();
    }

    public function get_display_name(): \lang_string {
        return new \lang_string('typeparticipateforum', 'block_gearup');
    }

    public function get_short_description(): \lang_string {
        return new \lang_string('typeparticipateforumdesc', 'block_gearup');
    }

    public function is_action_compatible(action $action): bool {
        return $action instanceof discussion_created
            || $action instanceof discussion_replied_to;
    }
    ...
    public function is_action_passing_constraints(action $action, objective_instance $instance,
        ...
        $uniqueness = $config->uniqueness ?? 0;
        if ($uniqueness > 0) {
            $contextid = (int) $action->get_context()->id;
            $discid = $this->get_discussion_id_from_action($action);
            $checkdisc = $uniqueness == 1;
            foreach ($state->seen as $seen) {
                if ($seen['contextid'] == $contextid) {
                    if (!$checkdisc) {
                        return false;   // Once per activity.
                    } else if ($discid == $seen['discid']) {
                        return false;   // Once per discussion.
                    }
                }
            }
        }
     ...
     * @return int
     */
    protected function get_discussion_id_from_action(action $action): int {
        $discid = $action->get_object_id();
        if ($action instanceof discussion_replied_to) {
            $discid = $action->get_discussion_id();
        }
        return (int) $discid;
    }
    ...
    protected function get_normalized_state(objective_instance $instance) {
        ...
        if ($state === null) {
            $state = (object) [
                'seen' => [], // Contains contextid and discid.
            ];
        }
        return $state;
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class participate_in_forum_config_form_extender implements extender {
    ...
    public function definition($mform): array {
        ...
        $els[] = $mform->addElement('select', 'cd_uniqueness', get_string('counts', 'block_gearup'), [
            ...
            1 => get_string('onceperdiscussion', 'block_gearup'),
            2 => get_string('onceperforum', 'block_gearup'),
        ]);
