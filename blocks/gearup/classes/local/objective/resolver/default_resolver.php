...
namespace block_gearup\local\objective\resolver;

use block_gearup\local\action\action;
use block_gearup\local\objective\type\type_with_action_consumption;
...
class default_resolver implements resolver {
    ...
    protected function get_type_name_from_class($classname) {
        return str_replace('block_gearup\\local\\objective\\type\\', '', $classname);
    }
    /**
     * Get all types.
     *
    ...
    public function get_types(): array {
        $this->init_types();
        $typenames = $this->lm->get_objective_types();
    ...
    public function get_types_compatible_with_action(action $action): array {
        ...
        return array_filter($this->types, function ($type) use ($action) {
            return $type->is_action_compatible($action);
        });
    }
    ...
    protected function init_types() {
        if (!isset($this->types)) {
            $types = [];
            $candidates = array_keys(\core_component::get_component_classes_in_namespace('block_gearup', 'local\objective\type'));
