...
use block_gearup\form\wizard\achievement_identity_form;
use block_gearup\form\wizard\challenge_identity_form;
use block_gearup\form\wizard\quest_identity_form;
use block_gearup\form\wizard\streak_identity_form;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mission_wizard_identity extends mission_route_base {
    ...
    protected function get_form() {
        if (!$this->form) {
            ...
            if ($this->is_quest()) {
                $this->form = new quest_identity_form($this->pageurl->out(false), [
                    'persistent' => $persistent,
                ]);
            } else if ($this->is_achievement()) {
                $this->form = new achievement_identity_form($this->pageurl->out(false), [
                    'persistent' => $persistent,
                ]);
            } else if ($this->is_challenge()) {
                $this->form = new challenge_identity_form($this->pageurl->out(false), [
                    'persistent' => $persistent,
                ]);
            } else if ($this->is_streak()) {
                $this->form = new streak_identity_form($this->pageurl->out(false), [
                    'persistent' => $persistent,
                ]);
    ...
    protected function get_subpage_html_head_title() {
        return get_string('missionidentity', 'block_gearup');
    }
