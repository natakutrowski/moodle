 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mission_users extends improved_table {
    ...
    public function init(): self {
        ...
        $columnsdefinition += [
            'instances' => get_string('success', 'block_gearup'),
        ];
    ...
    protected function prepare_query(): void {
        ...
        $this->query->annotate_mission_instance_count()
            ->annotate_mission_instance_100pc_count();
    ...
    public function col_instances($row) {
        return html_writer::tag('abbr', "{$row->mission_instance_100pc_count} / {$row->mission_instance_count}",
            ['title' => get_string('completed', 'block_gearup') . ' / ' . get_string('total', 'core')]);
    }
