 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quest_instructions_dynamic_form extends mission_dynamic_form_base {

    protected function definition() {
        parent::definition();
        $mform = $this->_form;

        $mform->addElement('textarea', 'instructions', get_string('questinstructions', 'block_gearup'), ['rows' => 5]);
        $mform->addHelpButton('instructions', 'storylineinstructions', 'block_gearup');
    ...
    protected function process_dynamic_save() {
        if ($data = $this->get_data()) {
            ...
            $textchanged = $beforerecord->instructions !== ($data->instructions ?? '');
            if ($textchanged) {
                $fs = get_file_storage();
                $files = $fs->get_area_files($this->get_mission()->get_context()->id,
                    'block_gearup',
                    'speech',
                ...
                foreach ($files as $file) {
                    if ($file->get_filepath() === '/instructions/') {
                        $file->delete();
                    }
                }
            }
