 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class outcome_dynamic_form extends persistent_dynamic_form {

    protected static $persistentclass = outcome::class;
    ...
    public function definition() {
        ...
        $isreward = $type instanceof user_facing_type;

        if ($isreward) {
            $mform->addElement('text', 'label', get_string('displayas', 'block_gearup'));
            $mform->addHelpButton('label', 'displayasoutcome', 'block_gearup');
            $mform->addRule('label', get_string('err_required', 'core_form'), 'required', null, 'client');
        } else {
        ...
        if (!$isreward) {
            $mform->setConstant('visibility', 0);
            $mform->addElement('static', 'visibilitytip', '',
                html_writer::div(get_string('outcomehiddenfromuser', 'block_gearup'), 'gu-text-sm')
            );
        } else {
            $mform->setConstant('visibility', 1);
        }
        ...
        if (is_object($data)) {
            ...
            if ($type && $extender) {
                $configdata = collection_utils::unprefix_data(
                    collection_utils::filter_data_with_prefix($extender->get_data($data), 'cd_'), 'cd_');
    ...
    protected function get_type_resolver() {
        return di::get('outcome_type_resolver');
    }
    ...
    protected function initialise_for_dynamic_submission(): void {
        ...
        } else if (!$mh->is_a_quest($mission) && !$mh->is_a_challenge($mission)) {
        ...
        if (!$id) {
            ...
            $outcomes = $mr->get_outcomes($mission->get_id());
            $outcome = null;
            ...
            $persistent = $outcome->get_persistent();
        }
    ...
    protected function check_access_for_dynamic_submission(): void {
        ...
        // Check whether the user is allowed to edit this.
