 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quest_description_dynamic_form extends mission_dynamic_form_base {

    protected function definition() {
        parent::definition();
        $mform = $this->_form;

        $mform->addElement('textarea', 'description', get_string('questdescription', 'block_gearup'), ['rows' => 5]);
    ...
    protected function process_dynamic_save() {
        if ($data = $this->get_data()) {
            ...
            $textchanged = $beforerecord->description !== ($data->description ?? '');
            if ($textchanged) {
                $fs = get_file_storage();
                ...
                foreach ($files as $file) {
                    if ($file->get_filepath() === '/description/') {
                        $file->delete();
                    }
                }
            }
