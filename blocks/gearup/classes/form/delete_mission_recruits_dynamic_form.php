...
class delete_mission_recruits_dynamic_form extends mission_dynamic_form_base {
    ...
    protected function definition() {
        ...
        $mform->addElement('html', html_writer::div(markdown_to_html(get_string('deleterecruitsintro', 'block_gearup',
            ['n' => $nrecruits])), 'gu-mb-4 last-of-type:[&_p]:gu-mb-0'));
        ...
        if ($ncompleted > 0) {
            ...
            $els = [];
            $els[] = $mform->createElement('select', 'deletecompleted', get_string('deletecompletedmissions', 'block_gearup'),
                [0 => get_string('no', 'core'), 1 => get_string('yes', 'core')]);
            $els[] = $mform->createElement('static', 'deletecompleted_help', '',
                html_writer::div($helpstring, 'gu-mt-2 gu-text-gray-500')
            );
            $mform->addElement('group', 'deletecompletedgrp', get_string('deletecompletedmissions', 'block_gearup'),
                $els, '', false);
        } else {
        ...
        if ($nassigners > 0) {
            $noptions++;
            $els = [];
            $els[] = $mform->createElement('select', 'deleteassigners', get_string('deleteassigners', 'block_gearup'),
                [0 => get_string('no', 'core'), 1 => get_string('yes', 'core')]);
            $els[] = $mform->createElement('static', 'deleteassigners_help', '',
    ...
    protected function process_dynamic_save() {
        if ($data = $this->get_data()) {
            ...
            if ($nrecruits !== $ninstances) {
                $message = get_string('successdeletedxrecruitsforymissions', 'block_gearup',
