...
final class complete_challenge_test extends base_testcase {
    ...
    public function test_basic(): void {
        ...
        $obj = $gudg->mock_objective($type, null);
     ...
     */
    public function test_which_constraints($context, $actioncontext, int $completedchallengeid, int $which,
            ?int $wantedchallengeid, $expected): void {
        $dg = $this->getDataGenerator();
        $gudg = $this->generator;
    ...
    public function test_initialise_state_which_specific(): void {
        ...
        $type = new complete_challenge();
        $missioninst = $gudg->mock_mission_instance($gudg->mock_quest());
        $obj = $gudg->mock_objective($type, ['missionid' => $cha1->get_id(), 'which' => complete_challenge::WHICH_SPECIFIC]);
        $objinst = $gudg->mock_objective_instance($obj, ['subjectid' => 2]);
