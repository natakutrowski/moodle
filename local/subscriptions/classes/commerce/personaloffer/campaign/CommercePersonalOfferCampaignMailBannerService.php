...
/**
 * Stores one optional public header banner per Personal Offer campaign.
 *
 * The campaign ID is the file itemid, so no additional DB column is required.
 */
final class CommercePersonalOfferCampaignMailBannerService {
    public const FILEAREA = 'personaloffer_campaign_banner';

    private const MAX_BYTES = 8 * 1024 * 1024;
    private const FILENAME = 'campaign-banner';
    ...
    public function save_uploaded_file(int $campaignid, array $upload): void {
        ...
        if ($error !== UPLOAD_ERR_OK || !is_uploaded_file((string)$upload['tmp_name'])) {
            throw new \moodle_exception(
                'commerce_personal_offer_campaign_banner_upload_error',
                'local_subscriptions'
            );
        }
        ...
        if ($size <= 0 || $size > self::MAX_BYTES) {
            throw new \moodle_exception(
                'commerce_personal_offer_campaign_banner_too_large',
                'local_subscriptions'
            );
        }
        ...
        if (!isset($extensions[$imagetype])) {
            throw new \moodle_exception(
                'commerce_personal_offer_campaign_banner_invalid_type',
                'local_subscriptions'
            );
        }
