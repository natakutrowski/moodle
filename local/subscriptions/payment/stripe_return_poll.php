...
try {
    ...
    // order: bind it to the purchase UUID and Stripe provider server-side.
    ...
    if (
        ...
        || $payment->get_provider() !== Provider::STRIPE
    ) {
        throw new CommerceOrderPresentationAccessDeniedException(
            'Payment does not belong to this Stripe order.'
        );
    }

    $result = (new StripeReturnPollingService(
    ...
    error_log(
        '[local_subscriptions][stripe_return_poll] ' .
        json_encode([
