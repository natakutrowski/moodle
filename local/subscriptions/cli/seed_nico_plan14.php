<?php
// local/subscriptions/cli/seed_nico_plan14.php
...
function parse_duration_key_days(string $key): int {
    ...
    // formats usuels: "1month", "3months", "1year", "3years", "6months", "1week", "1day"
    if (preg_match('/^(\d+)\s*(day|week|month|year)s?$/', $key, $m)) {
        $n = (int)$m[1];
        switch ($m[2]) {
            case 'day':   return max(1, $n);
            case 'week':  return max(1, 7 * $n);
            case 'month': return max(1, 30 * $n);   // approx
            case 'year':  return max(1, 365 * $n);  // approx
        }
    }
    // fallback 1 an
    return 365;
...
// 1) User (create or update basic fields)
...
$created[] = $e->id;
// Enrol so suspension has effect
$enrol($e);
...
mtrace("Tip: run 'php local/subscriptions/cli/run_subs_crons.php' in ~2 minutes, then:");
mtrace("     'php local/subscriptions/cli/show_nico_subs.php'");
