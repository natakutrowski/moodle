...
if ($options['help']) {
    echo "Phase 7.94C6 Native catalogue payment pipeline audit.\n\n";
    echo "Options:\n  --details\n  --strict\n  --json\n  --lang=fr\n";
    exit(0);
}

$report = (new CommerceCatalogFactory($DB))->payment_pipeline_auditor()->audit((string)$options['lang']);

if ($options['json']) {
    echo json_encode($report, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . PHP_EOL;
} else {
    cli_heading('Phase 7.94C6 - Native catalogue payment pipeline');
