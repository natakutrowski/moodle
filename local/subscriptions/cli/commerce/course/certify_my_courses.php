...
if (!empty($options['help'])) {
    echo <<<HELP
CampusFR Commerce My Courses and Native upgrade certification.
...
The command is read-only. It does not create purchases, grants, carts, or enrolments.
HELP;
    exit(0);
}
...
$markers = [
    CommerceMyCoursesCertificationFinding::OK => '[OK]',
    CommerceMyCoursesCertificationFinding::WARNING => '[WARN]',
    CommerceMyCoursesCertificationFinding::ERROR => '[FAIL]',
];

echo "============================================================\n";
echo "CampusFR Commerce — My Courses & Native Upgrades\n";
...
echo sprintf(
    ...
    $report->count(CommerceMyCoursesCertificationFinding::OK),
    $report->count(CommerceMyCoursesCertificationFinding::WARNING),
    $report->count(CommerceMyCoursesCertificationFinding::ERROR)
);
echo 'STATUS: ' . ($report->is_certified($strict) ? 'CERTIFIED' : 'NOT CERTIFIED') . PHP_EOL;
if (!$strict && $report->count(CommerceMyCoursesCertificationFinding::WARNING) > 0
        && $report->count(CommerceMyCoursesCertificationFinding::ERROR) === 0) {
