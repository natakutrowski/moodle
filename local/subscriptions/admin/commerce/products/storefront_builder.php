...
$builderarea = optional_param('area', 'content', PARAM_ALPHA);
...
$pageurl = new moodle_url(
    ...
    [
        ...
        'area' => $contentfocus ? 'content' : 'legacy',
    ]
);

$currentdefinition = $pageeditor->definition_from_product(
    $product,
    $editlanguage
);

CrmPageConfigurator::configure($PAGE, $context, $pageurl, get_string('commerce_storefront_editor_title', 'local_subscriptions'), 'local-subscriptions-commerce-product-storefront-builder-page');
...
if (data_submitted() && confirm_sesskey()) {
    ...
    $submitted = [
        ...
        'recommendations' => $contentfocus
            ? (string)($currentdefinition['recommendations'] ?? '')
            : optional_param(
    ...
    for (
        ...
        if (
            !in_array(
                $sectiontype,
                ['hero', 'rich_text', 'image_text', 'video', 'h5p', 'cta'],
                true
            )
        ) {
            continue;
...
foreach ($rows as $index => &$row) {
    ...
    $row['missingrequirement'] =
        $statusservice->missing_requirement($row);
...
echo CommerceProductEditorNavigationRenderer::breadcrumb($displayname, get_string('commerce_product_step_storefront', 'local_subscriptions'));
...
echo CommerceProductPageHeaderRenderer::render(
    ...
    CommerceDesignSystemRenderer::page_intro(
        get_string(
            'commerce_storefront_editor_intro',
            'local_subscriptions'
        )
    ),
    html_writer::link(
...
foreach ($storefronttabs as $key => $tab) {
    echo html_writer::link(
        ...
        [
            'class' => 'commerce-storefront-n812-tab'
                . ($key === 'builder' ? ' is-active' : ''),
            'aria-current' => $key === 'builder' ? 'page' : null,
        ]
    );
}
...
echo html_writer::div('', '', ['id' => 'n812-tools']);
...
echo html_writer::empty_tag('input', [
    'type' => 'hidden',
    'name' => 'area',
    'value' => $contentfocus ? 'content' : 'legacy',
]);
...
echo html_writer::empty_tag('input', ['type' => 'hidden', 'name' => 'sesskey', 'value' => sesskey()]);
echo html_writer::empty_tag('input', ['type' => 'hidden', 'name' => 'editlang', 'value' => $editlanguage]);
echo html_writer::div('', '', ['id' => 'n812-presentation']);
echo html_writer::start_div(
    'commerce-storefront-builder'
    . ($contentfocus ? ' commerce-storefront-builder--n813-content' : '')
);
...
// N8.17: merchandising, trust signals and discovery settings belong to Diffusion.
// Keep their current values in the form so saving Builder content never resets them.
echo html_writer::empty_tag('input', [
    'type' => 'hidden', 'name' => 'storefront_featured',
    'value' => $definition['featured'] ? 1 : 0,
]);
echo html_writer::empty_tag('input', [
    'type' => 'hidden', 'name' => 'storefront_displayorder',
    'value' => $definition['displayorder'],
]);
foreach ($definition['badges'] as $badge) {
    echo html_writer::empty_tag('input', [
        'type' => 'hidden', 'name' => 'storefront_badges[]', 'value' => $badge,
    ]);
}
echo html_writer::empty_tag('input', [
    'type' => 'hidden', 'name' => 'storefront_group', 'value' => $definition['group'],
]);
foreach ($definition['trust'] as $trustitem) {
    echo html_writer::empty_tag('input', [
        'type' => 'hidden', 'name' => 'storefront_trust[]', 'value' => $trustitem,
    ]);
}
echo html_writer::empty_tag('input', [
    'type' => 'hidden', 'name' => 'storefront_quickfacts', 'value' => $definition['quickfacts'],
]);
// Recommendations are owned by Diffusion. The Content Builder does not
// render or post their raw SKU representation.
...
$typeoptions = [
    '' => get_string(
        'commerce_storefront_section_empty',
        'local_subscriptions'
    ),
];
foreach (CommerceStorefrontPageEditor::section_types() as $type) {
    $typeoptions[$type] = get_string(
...
if ($contentfocus) {
    $n813types = [
        'hero' => [
            'icon' => 'fa-star',
            'desc' => 'commerce_storefront_n813_type_hero_help',
            'wire' => 'hero',
        ],
        'rich_text' => [
            'icon' => 'fa-align-left',
            'desc' => 'commerce_storefront_n813_type_rich_text_help',
            'wire' => 'text',
        ],
        'image_text' => [
            'icon' => 'fa-image',
            'desc' => 'commerce_storefront_n813_type_image_text_help',
            'wire' => 'split',
        ],
        'video' => [
            'icon' => 'fa-circle-play',
            'desc' => 'commerce_storefront_n813_type_video_help',
            'wire' => 'media',
        ],
        'features' => [
            'icon' => 'fa-table-cells-large',
            'desc' => 'commerce_storefront_n813_type_features_help',
            'wire' => 'cards',
        ],
        'cta' => [
            'icon' => 'fa-arrow-pointer',
            'desc' => 'commerce_storefront_n813_type_cta_help',
            'wire' => 'cta',
        ],
    ];

    echo html_writer::start_div(
        'card card-body mb-4 commerce-storefront-n813-add-block'
    );
    echo html_writer::tag(
        'h3',
        get_string(
            'commerce_storefront_n813_add_block_title',
            'local_subscriptions'
        ),
        ['class' => 'h5 mb-1']
    );
    echo html_writer::tag(
        'p',
        get_string(
            'commerce_storefront_n813_add_block_help',
            'local_subscriptions'
        ),
        ['class' => 'text-muted mb-3']
    );
    ...
    foreach ($n813types as $type => $meta) {
        ...
        echo html_writer::tag(
            ...
            html_writer::div(
                ...
                'commerce-storefront-n813-type-title'
            )
            . html_writer::div(
                '',
                'commerce-storefront-n813-wireframe commerce-storefront-n813-wireframe--'
                    . $meta['wire'],
                ['aria-hidden' => 'true']
            )
            . html_writer::tag(
            ...
            [
                'type' => 'button',
                'class' => 'commerce-storefront-n813-type-card',
                'data-n813-section-type' => $type,
            ]
        );
    }
    echo html_writer::end_div();
    ...
    echo html_writer::tag(
        ...
        html_writer::tag(
            'summary',
            get_string(
                'commerce_storefront_n813_more_blocks',
                'local_subscriptions'
            )
        )
        . html_writer::div(
            get_string(
                'commerce_storefront_n813_more_blocks_help',
                'local_subscriptions'
            ),
            'form-text mt-2'
        ),
        ['class' => 'commerce-storefront-n813-more-types mt-3']
    );
    echo html_writer::end_div();
}

echo html_writer::start_div(
    'commerce-storefront-builder__toolbar card card-body mb-4'
    . ($contentfocus ? ' commerce-storefront-builder__toolbar--n813-fallback' : '')
);
...
echo html_writer::start_div('commerce-storefront-preview-canvas', [
...
foreach ($rows as $index => $row) {
    ...
    $missingrequirement = (string)($row['missingrequirement'] ?? '');
    $statusattributes = ['data-section-readiness' => '1'];
    if ($missingrequirement !== '') {
        $statusattributes['title'] = get_string(
            'commerce_storefront_n818_missing_' . $missingrequirement,
            'local_subscriptions'
        );
    }
    echo html_writer::span(
    ...
    echo html_writer::end_div();
    echo html_writer::start_tag('details', [
        'class' => 'commerce-storefront-n818-preview mb-3',
    ...
    echo html_writer::tag(
        ...
        '<i class="fa-solid fa-eye me-2" aria-hidden="true"></i>'
            . get_string(
                'commerce_storefront_n817_block_preview',
                'local_subscriptions'
            ),
        ['class' => 'commerce-storefront-n818-preview__summary']
    );
    echo html_writer::start_div(
        'commerce-storefront-n819-preview-toolbar'
    );
    echo html_writer::span(
        get_string(
            'commerce_storefront_n819_preview_device',
            'local_subscriptions'
        ),
        'commerce-storefront-n819-preview-device-label'
    );
    echo html_writer::start_div('btn-group btn-group-sm', [
        ...
        'data-region' => 'storefront-block-preview-switcher',
    ]);
    foreach ([
        'desktop' => 'fa-desktop',
        ...
        echo html_writer::tag(
            ...
            [
                ...
                'aria-pressed' => $device === 'desktop'
                    ...
                    : 'false',
                'title' => get_string(
                    'commerce_storefront_preview_' . $device,
                    'local_subscriptions'
                ),
            ]
        );
    ...
    echo html_writer::end_div();
    echo html_writer::start_div(
        'commerce-storefront-section-card__preview '
            . 'commerce-storefront-section-card__preview--'
            . $row['type']
    ...
    if ($contentfocus) {
        ...
        $previewsubtitle = trim((string)($row['subtitle'] ?? ''));
        $previewcontent = trim(strip_tags((string)($row['content'] ?? '')));
        $previewimage = is_array($row['imagemediadiagnostic'] ?? null)
            ? (string)($row['imagemediadiagnostic']['url'] ?? '')
            : '';
        $previewclass = 'commerce-storefront-n813-live-preview commerce-storefront-n813-live-preview--'
            . $row['type'];
        if ($row['type'] === 'image_text') {
            $previewclass .= ' is-image-' . ($row['imageposition'] ?? 'left');
        }
        if ($row['type'] === 'hero') {
            $previewclass .= ' is-hero-' . ($row['herolayout'] ?? 'text_media');
        }
        echo html_writer::start_div($previewclass);
        if (
            in_array($row['type'], ['hero', 'image_text'], true)
            && $previewimage !== ''
        ) {
            echo html_writer::div(
                html_writer::empty_tag('img', [
                ...
                'commerce-storefront-n813-live-preview__media'
            );
        } else if (in_array($row['type'], ['hero', 'image_text', 'video'], true)) {
            echo html_writer::div(
                html_writer::tag('i', '', [
                    'class' => 'fa-solid fa-image',
                    'aria-hidden' => 'true',
                ]),
                'commerce-storefront-n813-live-preview__media is-placeholder'
            );
        }
        echo html_writer::start_div(
            'commerce-storefront-n813-live-preview__copy'
        );
        if ($previewtitle !== '') {
            echo html_writer::tag(
                'strong',
                s($previewtitle),
                ['class' => 'commerce-storefront-n813-live-preview__title']
            );
        }
        if ($previewsubtitle !== '') {
            echo html_writer::tag(
                'span',
                s($previewsubtitle),
                ['class' => 'commerce-storefront-n813-live-preview__subtitle']
            );
        }
        if ($previewcontent !== '') {
            echo html_writer::div(
                format_text((string)($row['content'] ?? ''), FORMAT_HTML, ['filter' => false]),
                'commerce-storefront-n817-preview-content'
            );
        }
        if (in_array($row['type'], ['features', 'faq'], true) && trim((string)$row['items']) !== '') {
            echo html_writer::start_div('commerce-storefront-n817-preview-items');
            foreach (preg_split('/\R/u', (string)$row['items']) ?: [] as $previewitem) {
                $previewitem = trim($previewitem);
                if ($previewitem === '') {
                    continue;
                }
                $separator = str_contains($previewitem, '|||') ? '|||' : '|';
                [$left, $right] = array_pad(explode($separator, $previewitem, 2), 2, '');
                echo html_writer::div(
                    html_writer::tag('strong', s(trim($left)))
                    . html_writer::tag('span', s(trim($right))),
                    'commerce-storefront-n817-preview-item'
                );
            }
            echo html_writer::end_div();
        }
    ...
    if (!$contentfocus) {
        if ($row['type'] === 'image_text'
            && !empty($row['imagemediadiagnostic']['url'])) {
            ...
            && !empty($row['videomediadiagnostic']['url'])) {
            echo html_writer::tag('video', '', [
                'src' => $row['videomediadiagnostic']['url'],
                'controls' => 'controls',
                'preload' => 'metadata',
            ]);
        } else if ($row['type'] === 'h5p'
            && !empty($row['h5pmediadiagnostic'])) {
            echo html_writer::div(
                html_writer::tag('i', '', [
                ...
                . html_writer::span(
                    s((string)$row['h5pmediadiagnostic']['filename'])
                ),
    ...
    if ($contentfocus) {
        echo html_writer::start_div(
            'commerce-storefront-n813-layout-guide'
        );
        echo html_writer::tag(
            'h4',
            get_string(
                'commerce_storefront_n813_layout_choice_title',
                'local_subscriptions'
            ),
            ['class' => 'h6 mb-1']
        );
        ...
        if ($row['type'] === 'hero') {
            ...
            foreach ([
                'text_media' => 'commerce_storefront_hero_layout_text_media',
                'media_text' => 'commerce_storefront_hero_layout_media_text',
                'stacked' => 'commerce_storefront_hero_layout_stacked',
                'overlay' => 'commerce_storefront_hero_layout_overlay',
            ] as $value => $labelkey) {
                echo html_writer::tag(
                    'button',
                    html_writer::div(
                        '',
                        'commerce-storefront-n813-layout-sketch commerce-storefront-n813-layout-sketch--hero-'
                            . $value,
                        ['aria-hidden' => 'true']
                    )
                    . html_writer::span(
                    ...
                    [
                        'type' => 'button',
                        'class' => 'commerce-storefront-n813-layout-card'
                            . ($row['herolayout'] === $value ? ' is-selected' : ''),
                        'data-n813-control' => 'section_hero_layout_' . $index,
                        'data-n813-value' => $value,
                    ]
                );
            }
            echo html_writer::end_div();
            ...
            echo html_writer::start_div(
                'commerce-storefront-n813-layout-grid commerce-storefront-n813-layout-grid--two'
            );
            foreach ([
                'left' => 'commerce_storefront_image_position_left',
                'right' => 'commerce_storefront_image_position_right',
            ] as $value => $labelkey) {
                echo html_writer::tag(
                    'button',
                    html_writer::div(
                        '',
                        'commerce-storefront-n813-layout-sketch commerce-storefront-n813-layout-sketch--image-'
                            . $value,
                        ['aria-hidden' => 'true']
                    )
                    . html_writer::span(
                    ...
                    [
                        'type' => 'button',
                        'class' => 'commerce-storefront-n813-layout-card'
                            . ($row['imageposition'] === $value ? ' is-selected' : ''),
                        'data-n813-control' => 'section_image_position_' . $index,
                        'data-n813-value' => $value,
                    ]
                );
            }
            ...
            echo html_writer::div(
                get_string(
                    'commerce_storefront_n813_layout_automatic_help',
                    'local_subscriptions'
                ),
                'commerce-storefront-n813-layout-auto'
            );
        }
    ...
    if ($contentfocus) {
        echo html_writer::start_tag('details', [
            'class' => 'commerce-storefront-n816-advanced mb-4',
        ]);
        echo html_writer::tag(
            'summary',
            html_writer::tag('i', '', [
                'class' => 'fa fa-sliders me-2',
                'aria-hidden' => 'true',
            ]) . get_string('commerce_storefront_n816_advanced_options', 'local_subscriptions'),
            ['class' => 'commerce-storefront-n816-advanced-summary']
        );
    }
    ...
    echo html_writer::tag(
        'h4',
        $contentfocus
            ? get_string('commerce_storefront_n813_advanced_layout_title', 'local_subscriptions')
            : get_string('commerce_storefront_composer_layout', 'local_subscriptions'),
        ['class' => 'h6 mb-1']
    );
    if ($contentfocus) {
        echo html_writer::tag(
            'p',
            get_string(
                'commerce_storefront_n813_advanced_layout_help',
                'local_subscriptions'
            ),
            ['class' => 'text-muted small mb-3']
        );
    ...
    // Stable technical values are preserved silently; normal editing does not need them.
    echo html_writer::empty_tag('input', [
        'type' => 'hidden',
    ...
    // The section type is chosen when the block is created; do not ask for it again.
    echo html_writer::empty_tag('input', [
        'type' => 'hidden',
        'name' => 'section_type_' . $index,
        'value' => $row['type'],
    ]);
    $fields = [
        ['col-lg-6', 'section_title_' . $index, get_string('commerce_storefront_section_title', 'local_subscriptions'), 'text'],
        ['col-lg-6', 'section_subtitle_' . $index, get_string('commerce_storefront_section_subtitle', 'local_subscriptions'), 'text'],
    ];
    ...
    if (in_array($row['type'], ['features', 'faq'], true)) {
        // Structured blocks are entirely driven by their item collection.
        // Preserve any historical generic content without showing a redundant
        // editor in the normal workflow.
        echo html_writer::empty_tag('input', [
            'type' => 'hidden',
    ...
    // Type-specific secondary fields only: avoid generic fields that make every block look complex.
    if (in_array($row['type'], ['video', 'h5p'], true)) {
        echo html_writer::start_div('col-lg-6');
        ...
        echo html_writer::empty_tag('input', ['type' => 'hidden', 'name' => 'section_auxiliary_' . $index, 'value' => $row['auxiliary']]);
    }
    ...
    if (in_array($row['type'], ['hero', 'image_text'], true)) {
        ...
        echo html_writer::empty_tag('input', ['type' => 'hidden', 'name' => 'section_alt_' . $index, 'value' => $row['alt']]);
    }

    if (in_array($row['type'], ['features', 'faq'], true)) {
        $itemslabelkey = $row['type'] === 'faq'
            ? 'commerce_storefront_n818_faq_items'
            : 'commerce_storefront_n818_feature_items';
        $itemshelpkey = $row['type'] === 'faq'
            ? 'commerce_storefront_n818_faq_items_help'
            : 'commerce_storefront_n818_feature_items_help';
        echo html_writer::start_div('col-12');
        echo html_writer::tag(
            'label',
            get_string($itemslabelkey, 'local_subscriptions'),
            [
                'for' => 'section_items_' . $index,
                'class' => 'form-label',
            ]
        );
        echo html_writer::tag(
        ...
        echo html_writer::tag(
            'div',
            get_string($itemshelpkey, 'local_subscriptions'),
            ['class' => 'form-text']
        );
        echo html_writer::end_div();
    } else {
        echo html_writer::empty_tag('input', ['type' => 'hidden', 'name' => 'section_items_' . $index, 'value' => $row['items']]);
    }
...
(function() {
    var fallbackSelect = document.querySelector(
        'select[name="builder_type"]'
    );
    var addButton = document.querySelector(
        'button[name="builder_action"][value="add"]'
    );
    ...
    document.querySelectorAll(
        ...
        card.addEventListener('click', function() {
            if (!fallbackSelect || !addButton) {
                return;
            }
            fallbackSelect.value = card.getAttribute(
                'data-n813-section-type'
            );
            addButton.click();
        });
    ...
    document.querySelectorAll(
        ...
        card.addEventListener('click', function() {
            var name = card.getAttribute('data-n813-control');
            var value = card.getAttribute('data-n813-value');
...
(function() {
    ...
    function setBlockPreviewDevice(button) {
        var device = button.getAttribute('data-block-preview-device');
        if (['desktop', 'tablet', 'mobile'].indexOf(device) === -1) {
            return;
        }

        var card = button.closest('[data-region="storefront-section-card"]');
        if (!card) {
            return;
        }
        ...
        preview.setAttribute('data-block-preview-mode', device);

        card.querySelectorAll(
            '[data-block-preview-device]'
        ).forEach(function(candidate) {
            var active =
                candidate.getAttribute('data-block-preview-device') === device;
    ...
    root.addEventListener('click', function(event) {
        var button = event.target.closest('[data-block-preview-device]');
        if (!button) {
            return;
        }
        event.preventDefault();
        event.stopPropagation();
        setBlockPreviewDevice(button);
    });

    root.addEventListener('keydown', function(event) {
        var button = event.target.closest('[data-block-preview-device]');
        if (!button || ['ArrowLeft', 'ArrowRight'].indexOf(event.key) === -1) {
            return;
        }
        ...
        var buttons = Array.prototype.slice.call(
            group.querySelectorAll('[data-block-preview-device]')
        );
        ...
        buttons[next].focus();
        setBlockPreviewDevice(buttons[next]);
    });
})();
JS);
