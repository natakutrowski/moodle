...
if ($form->is_cancelled()) { redirect(new moodle_url('/local/subscriptions/admin/commerce/plans/index.php')); }
if ($data = $form->get_data()) {
    $save = (object)[
        'id' => (int)($data->id ?? 0), 'name' => trim($data->name), 'accessscopeid' => (int)$data->accessscopeid,
        'duration_key' => $data->duration_key, 'highlight_type' => in_array($data->highlight_type ?? '', ['popular','premium'], true) ? $data->highlight_type : null,
        'is_recurring' => (int)$data->is_recurring, 'last_update' => time(),
    ];
    if ($save->id) { $DB->update_record('subscription_plan', $save); }
    else { $save->creation_date = time(); $save->is_active = 0; $save->id = $DB->insert_record('subscription_plan', $save); }
