 ...
 */
class database_entry_created_reason extends \block_xp\local\reason\database_entry_created_reason implements reason_with_location {
    ...
    public function get_location_url() {
        ...
        if (!\block_xp\di::get('db')->record_exists('data_records', ['id' => (int) $this->get_object_id()])) {
            return context_utils::get_url($context);
        }
        return new moodle_url('/mod/data/view.php', [
            'id' => (int) $context->instanceid,
            'rid' => (int) $this->get_object_id(),
            'mode' => 'single',
        ]);
    }
